...
void CPropertiesWnd::AddProperty(RBX::Instance* i, rttr::property p)
{
	...
	{
		property = new CMFCPropertyGridProperty(name.c_str(), (_variant_t)v.to_float(), "");
	}

	if (v.get_type() == rttr::type::get<double>())
	{
		property = new CMFCPropertyGridProperty(name.c_str(), (_variant_t)v.to_double(), "");
	}

	if (v.get_type() == rttr::type::get<G3D::Vector3>())
	{
		G3D::Vector3 v3 = v.convert<G3D::Vector3>();

		property = new CMFCPropertyGridProperty(name.c_str(), 0, 1);

	}

	if (v.get_type() == rttr::type::get<G3D::Color3>())
	{
